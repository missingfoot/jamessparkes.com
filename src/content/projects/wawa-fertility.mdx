...
import calLinkedPoster from '../../assets/images/wawa-cal-linked-poster.jpg';
import calPanningPoster from '../../assets/images/wawa-cal-panning-poster.jpg';
import calSideBarPoster from '../../assets/images/wawa-cal-side-bar-poster.jpg';
import reportingPoster from '../../assets/images/wawa-reporting-poster.jpg';
import wawaAiPoster from '../../assets/images/wawa-ai-poster.jpg';
import onboardingPoster from '../../assets/images/wawa-onboarding-poster.jpg';
...
<KVGroup>
  <KV term="Losing work on missing dependencies" desc="Creating something as simple as a product required a medical code and a tax rate to already exist. If they didn't, the person configuring the clinic had to abandon the form, go create the missing dependency elsewhere, and start again, losing their work each time." />
  <KV term="Bulk import was hidden" desc="A faster path existed for clinics with large catalogues, but it was admin-only and undocumented, with no template or guidance on the expected file structure, so the clinics who needed it most often didn't know it existed." />
  <KV term="Settings were disconnected" desc="Related concepts (medications and products, tax rates and price lists) lived in separate, unrelated parts of the settings area." />
  <KV term="Training didn't scale" desc="Larger clinics were still relying on multi-day, in-person training to get set up at all, which doesn't scale past a handful of rollouts a year." />
</KVGroup>

<InlineVideo src="/videos/wawa-onboarding.mp4" poster={onboardingPoster} alt="Walking through the guided clinic onboarding flow" />
...
<KVGroup>
  <KV term="The problem" desc="Extending a single medication on the tracking sheet took seven or more clicks (open edit, scroll, change the date, save, wait for the page to refresh and re-scroll to where you were). A patient on five medications meant repeating that five times, several times a day, across many patients at once." />
  <KV term="A useful benchmark" desc="A clinician showed a competitor product that solved the same problem with a double-click on the target date, useful as a benchmark, not a template to copy outright." />
  <KV term="Redesigned interaction" desc="Reworked medication extension around direct manipulation instead of a multi-step edit flow." />
  <KV term="Related task list rework" desc="Separately reworked the day-to-day clinical task list (used to sign off consents and complete clinical steps) around an email-inbox-style model: auto-advance to the next item, keyboard shortcuts, and default filters like &quot;overdue&quot; and &quot;today&quot;, since this was a workflow clinicians touched dozens of times a day, not a once-a-month feature." />
</KVGroup>

### Donor flow

<KVGroup>
  <KV term="The problem" desc="The donor management flow had accumulated as a series of individual fixes to specific client-reported issues rather than being designed as one coherent workflow. It showed: a donor reservation system that felt bolted on, and inventory visibility that existed but was buried in the wrong part of the product entirely." />
  <KV term="Redesigned as one section" desc="Built a proper &quot;Donors&quot; section inside cryo storage, with a filterable table for matching donors against criteria clinics actually search on." />
  <KV term="Consolidated dashboard" desc="A donor dashboard surfaces family limits, storage, and any holds or restrictions in one place rather than scattered across separate screens." />
</KVGroup>

### Lab sheets

<KVGroup>
  <KV term="The context" desc="Embryology labs run on printed lab sheets, physically carried between the bench, the incubator, and storage." />
  <KV term="First framing" desc="Auto-generate a pre-filled PDF from a patient's active cycle so staff could print it, fill it in by hand, and scan it back in." />
  <KV term="The problem with that plan" desc="Filling in a printed sheet by hand and then re-entering that same data digitally is exactly the kind of duplicate data entry clinics are already drowning in." />
  <KV term="The pivot" desc="Shifted the design to treat the lab sheet as a live view of data that already exists in wawa rather than a form to fill out, with a QR code as the bridge back to the physical world: scan the code on a sample or tray, and the corresponding digital sheet opens directly, already populated." />
</KVGroup>
...
<KVGroup>
  <KV term="&quot;Referrals&quot; was too narrow" desc="Patients arrive via referral letters, but also web booking forms, direct sign-ups, and marketing channels, and all of them needed the same intake workflow." />
  <KV term="Leads mixed into patients" desc="Patients created from these different sources were mixed into the main patient table with no way to tell them apart." />
  <KV term="No duplicate detection" desc="There was no way to catch a duplicate patient being entered twice." />
  <KV term="No expiry visibility" desc="Referrals with a natural expiry window had no dedicated view surfacing which ones were about to lapse, so staff had to know to go looking for them." />
</KVGroup>
...
## Project: Conversational AI booking and scheduling

### Overview

For wawa's clinic platform, I designed an AI feature that moves beyond static documentation into task execution. Rather than just answering questions, it lets clinic staff book appointments and build schedules using natural language, understanding intent even from loosely phrased or misspelled input.

For appointment booking, staff can type a request like booking a patient for an extended consultation, and the AI parses the details, cross-references the patient record, and surfaces a confirmation step (such as verifying date of birth) to catch mishears or near-matches on names. It then walks through location, available days, time slots, and appointment type, before logging the booking directly to the calendar.

For scheduling, a known pain point for clinic staff, I applied the same approach: users can describe a new schedule in plain language (staff member, room, start date, frequency, pauses) and the AI interprets it and creates the schedule entry directly, removing the need to manually configure it through a traditional form-based interface.

<InlineVideo src="/videos/wawa-ai.mp4" poster={wawaAiPoster} alt="Booking an appointment and building a staff schedule using natural language with wawa's AI assistant" class="video-capped" />
...
<KVGroup>
  <KV term="Four regulators, four formats" desc="Fertility clinics must report clinical data to different regulatory bodies depending on jurisdiction (HFEA/PRISM in the UK, ANZARD in Australia/NZ, SART in the US, and BORN), each with its own data requirements, formats, and submission mechanism." />
  <KV term="Fully disconnected task" desc="Reporting was bolted onto the end of clinical work: staff re-entered data that often already existed elsewhere in the clinic's systems." />
  <KV term="No outstanding-submission visibility" desc="No way to see which patients still had outstanding submissions short of running a manual report and checking each record by hand, and no reminders for approaching deadlines despite real regulatory consequences for missing them." />
  <KV term="Real error rates" desc="Clinics estimated meaningful error rates under this manual process, which lines up with public regulatory data: the UK's HFEA board has reported clinics submitting through third-party integrations average a 6 to 8 percent error rate, against under 1 percent for clinics submitting directly." />
</KVGroup>

### Approach

<KVGroup>
  <KV term="Rejected the obvious approach" desc="Reached with the product lead early on: don't build dedicated &quot;reporting forms&quot; for staff to fill in, since that just relocates the same manual data entry problem into a new screen." />
  <KV term="Verification, not forms" desc="wawa already captures the underlying clinical data (registrations, cycles, procedures, outcomes) as a byproduct of normal clinical workflow, so the reporting feature became a dashboard over that existing data showing what's ready, what's missing, and what's invalid, with submission reduced to a single action once a record is complete." />
  <KV term="Canonical data, per-jurisdiction mapping" desc="Each data field is stored once in a canonical form and mapped to whatever format a given regulator expects only at submission time, so the same underlying record can satisfy multiple regulators' differing field formats without duplicating data entry per jurisdiction." />
</KVGroup>

<InlineVideo src="/videos/wawa-reporting.mp4" poster={reportingPoster} alt="The regulatory reporting dashboard showing patient submission status across ready, needs attention, overdue, and submitted states" />
...
<KVGroup>
  <KV term="No recurring unavailability" desc="The scheduler had no way to express recurring unavailability (lunch breaks, non-bookable hours) or restrict which appointment types could be booked in which windows, which for at least one clinic was serious enough to threaten their go-live timeline." />
  <KV term="No linked, multi-resource bookings" desc="A clinic walked through a &quot;treatment scan&quot; appointment type that needs to book two different resources with a staggered offset: fifteen minutes on a sonographer's diary, followed automatically by fifteen minutes on the phlebotomy list right after, booked as one unit. The existing scheduler had no concept of a single appointment type driving multiple linked, offset resource bookings." />
</KVGroup>

<InlineVideo src="/videos/wawa-cal-linked.mp4" poster={calLinkedPoster} alt="Booking a treatment scan appointment that links two staggered, offset resource bookings into one unit" />

<InlineVideo src="/videos/wawa-cal-side-bar.mp4" poster={calSideBarPoster} alt="Navigating the scheduler's sidebar" />

<InlineVideo src="/videos/wawa-cal-panning.mp4" poster={calPanningPoster} alt="Panning across the scheduler calendar" />
